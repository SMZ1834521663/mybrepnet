...
class BRepNetDataset(Dataset):
    ...
    def load_body(self, idx):
        ...
        Ce = self.build_coedges_of_edges_tensor(body_data)  #半边对边的关系

        # Try building point grids from the left coedges 
        # of each edge
        Ge = self.build_edge_grids_from_left_coedges(Gc, Ce, body_data)    #################################返回GC的一半，只找正边的特征就是Ge

        Cf, Csf, new_to_old_face_indices = self.build_coedges_of_faces_tensor(   #new to old是所有面的索引排序，包括大面和小面，就是【cf，csf】
            body_data, 
    ...
    def build_kernel_tensors(self, body_data):   #这里获取ke什么的，加上offset之后得到data里的最终的结果
        n = body_data["coedge_to_next"]               #这是一个npz文件里的
    ...
    def build_edge_grids_from_left_coedges(self, Gc, Ce, body_data):
        ...
        for edge_index in range(num_edges):
            ...
            if reverse_flags[second_coedge_index]==1:
                ...
                left_coedge_indices[edge_index] = second_coedge_index   #选完正向的边，n个
        return Gc[left_coedge_indices]  #返回
    ...
    def	build_kernel_tensor_from_topology(self, n, p, m, e, f, kernel):
        ...
        for walk_instructions in kernel:
            ...
            for instruction in walk_instructions:
                if instruction == "n":   #c是索引变换，一开始代表全局索引，然后比如next，代表所有半边的下一个半边索引
                    c = n[c]
                elif instruction == "p":
                    c = p[c]
                elif instruction == "m":
    ...
    def build_coedges_of_faces_tensor(
        ...
        for coedge_index, face_index in enumerate(coedge_to_face):
            ...
            face_to_coedges[face_index].append(coedge_index)    #获得面对半边
        ...
        for face_index in range(num_faces):
            ...
            if len(face_to_coedges[face_index]) > max_coedges_per_face:    #大于30个半边的面叫大面，大面有特殊处理
        ...
        face_permutation = torch.cat([small_face_indices, big_face_indices])   #所有面的该项目索引排序
...
def brepnet_collate_fn(data_list):
    ...
    #面特征，7大类，是拓扑学的分类结果，原先是onehot形式，不过归一化了
    #面网格特征，是通过uvnet后的结果
    #边特征，10类拓扑学分类结果，也是归一化了
    #边网格特征，是通过uvnet后的结果.
    #半边特征，1类拓扑学分类结果，实际上就是是否反向，也做了归一化
    #半边网格特征，是通过uvnet后的结果
    #-Rep对齐的特殊坐标系
    #面核：核都是半半边坐标变换：与B边对什么的转换
    #边核：
    #半边核：
    #半边对边的索引
    #半边对小面，这里面保存的是面的对应的半边索引
    #半边对大面
    #标签，seg文件里的，对应八大类，论文图中有提到
    #每一个batch对应的边，半边，面的全局索引
    #npz文件名称
    batch_data = {
        "face_features": concatenate_tensor_arrays(Xf_small_faces, Xf_big_faces, unsqueeze=True), #n,7     #bigfaces是11个，n=2451
        "face_point_grids": concatenate_tensor_arrays(Gf_small_faces, Gf_big_faces, unsqueeze=True),#n,7,10,10，n=2451
        "edge_features": torch.cat(Xe),#n,10，n=5667
        "edge_point_grids": torch.cat(Ge),#n,12,10，n=5667
        "coedge_features": torch.cat(Xc),#n,1，n=11325
        "coedge_point_grids": torch.cat(Gc),#n,12,10，n=11325
        "coedge_lcs": torch.cat(lcs),#n,4,4，n=11325
        "face_kernel_tensor": torch.cat(Kf),#n,2，n=11325
        "edge_kernel_tensor": torch.cat(Ke),#n,5，n=11325
        "coedge_kernel_tensor": torch.cat(Kc),#n,6，n=11325
        "coedges_of_edges": torch.cat(Ce),#n,2，n=5667
        "coedges_of_small_faces": torch.cat(Cf),#n,30  ，这两个都不确定大小，2440
        "coedges_of_big_faces": Csf,#n,11  这两个都不确定大小，11
        "labels": concatenate_tensor_arrays(labels_small_faces, labels_big_faces, unsqueeze=False),#n，和面特征一样的n 2451
        "split_batch": split_batch,#200个边索引，共边索引，面索引（2440个，而面特征2451个，由于bigface11个的存在）
