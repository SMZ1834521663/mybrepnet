...
def calc_cano_weight_volume(data_dir, gender = 'neutral'):
    smpl_params = np.load(data_dir + '/smpl_params.npy', allow_pickle=True).item()
    smpl_shape = torch.from_numpy(smpl_params['shapes'][0]).to(torch.float32)
    ...
    smpl_model = smplx.SMPL(model_path='models/SMPL_NEUTRAL.pkl')
...
if __name__ == "__main__":
    path = 'data/zju_mocap/CoreView_394/lbs'
    calc_cano_weight_volume(path)