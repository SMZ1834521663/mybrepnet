...
class PointMeshDistance(Function):
    ...
    def forward(ctx, points, tris, n_batch=1):
        ...
        min_triangle_area = torch.tensor(0, dtype=torch.float32, device = device)
        zeros = torch.zeros(n_batch, dtype=torch.int64, device=device)
        dists, idxs = _C.point_face_dist_forward(
            points, zeros, tris, zeros, n_points, min_triangle_area
        )
...
def points_to_barycentric(points: torch.Tensor, triangles: torch.Tensor, method="cramer", eps: float = 1e-16):
    ...
    def constraint_barycentric(barycentric: torch.Tensor):
        ...
        barycentric = barycentric / (torch.sum(barycentric, dim = 1, keepdim = True)) 
...
def random_sample(bounds, point_num):
    random_points = torch.rand(point_num, 3).to(bounds)
    random_points = bounds[0] + random_points * (bounds[1] - bounds[0])
...
def sample_blend_closest_points(src: torch.Tensor, ref: torch.Tensor, values: torch.Tensor, K: int = 5, exp: float = 1e-8):
    #find 5 nearest points on mesh and weighted sum 
    n_batch, n_points, _ = src.shape
...
def points_to_local(points: torch.Tensor, triangles: torch.Tensor):
    # points: n_points, 3
    # triangles: n_points, 3, 3
    return points[:,None,:] - triangles
    


def sample_local_feature_on_surface(points: torch.Tensor, verts: torch.Tensor, faces: torch.Tensor, values: torch.Tensor):
    #points: n_batch, n_points, 3
    # verts: n_batch, n_verts, 3
    # faces: n_batch, n_faces, 3 (long)
    # values: n_batch, n_verts, D (might need to augment some) 
    ...
    values = values.view(-1, D)
    dists, face_ids = point_mesh_distance(points, verts[faces], n_batch)
    local_coords = points_to_local(points, verts[faces[face_ids]])
    feature = values[face_ids]
    # bary = points_to_barycentric(points, verts[faces[face_ids]])  # (n, 3)
    # interp = torch.sum(values[faces[face_ids]] *  # (n, 3, 3)
                    #    bary[..., None],  # (n, 3, 1)
                    #    dim=1)

    return feature.view(n_batch, n_points, -1), face_ids.view(n_batch, n_points, 1), local_coords.view(n_batch, n_points, 9)


def sample_local_feature_on_surface_dis(points: torch.Tensor, verts: torch.Tensor, faces: torch.Tensor, values: torch.Tensor):
    #points: n_batch, n_points, 3
    # verts: n_batch, n_verts, 3
    # faces: n_batch, n_faces, 3 (long)
    # values: n_batch, n_verts, D (might need to augment some) 
    ...
    return feature.view(n_batch, n_points, -1), face_ids.view(n_batch, n_points, 1), local_coords.view(n_batch, n_points, 9), dists.view(n_batch, n_points, 1)



def sample_facesid_on_surface(points: torch.Tensor, verts: torch.Tensor, faces: torch.Tensor):
    #points: n_batch, n_points, 3
    # verts: n_batch, n_verts, 3
    # faces: n_batch, n_faces, 3 (long)
    # values: n_batch, n_verts, D (might need to augment some) 
    ...
    faces = faces.view(-1, 3)
    dists, face_ids = point_mesh_distance(points, verts[faces], n_batch)
    verts_idx = faces[face_ids]
    bary = points_to_barycentric(points, verts[faces[face_ids]])  # (n, 3)
    return verts_idx,  dists.view(n_batch, n_points, 1), bary


# if __name__ == "__main__":
#     bounds = torch.tensor([[-1, -1, -1], [1, 1, 1]])
#     point_num = 30000
#     rand_point = random_sample(bounds, point_num)
