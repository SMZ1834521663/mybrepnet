...
class Dataset(data.Dataset):
    def __init__(self, data_root, human, ann_file, split):
        ...
        self.big_A = self.load_bigpose()
        self.tbounds = if_nerf_dutils.get_bounds(self.tvertices)
    ...
    def prepare_input(self, i):
        ...
        return Rh, Th, poses#, nearest_frame_index, canonical_joints 
    ...
    def __getitem__(self, index):
        ...
        if cfg.white_bg:
            ...
            img[msk == 0] = 0
        K[:2] = K[:2] * cfg.ratio
        coords = np.where(msk == True)
        min_xy = np.min(coords, axis=1) 
        max_xy = np.max(coords, axis=1)
        bound_mask = np.concatenate([min_xy, max_xy], axis = 0)
        ...
        Rh, Th, poses = self.prepare_input(i)
...
def loadZjuMocap(dataset: Dataset):
    zjudata = []
    ...
    sceneInfo = {
        ...
        'tvertices': dataset.tvertices,
        'joints': dataset.joints,
        'faces': dataset.faces,
        'parents': dataset.parents,
        'tbounds': dataset.tbounds
    }
    return sceneInfo

if __name__ == "__main__":
    import torch.nn.functional as F
    from lib.utils.blend_utils import tpose_points_to_pose_points, pose_points_to_tpose_points
    ...
    big_pose = tpose_points_to_pose_points(torch.from_numpy(dataset.tvertices[None]), torch.from_numpy(dataset.weights[None]).permute(0, 2, 1), torch.from_numpy(dataset.big_A[None]))
    T = pose_points_to_tpose_points(big_pose, torch.from_numpy(dataset.weights[None]).permute(0, 2, 1), torch.from_numpy(dataset.big_A[None]))
    write_ply('tmp/t.ply', T[0].detach().numpy())
    write_ply('tmp/big.ply', big_pose[0].detach().numpy())
