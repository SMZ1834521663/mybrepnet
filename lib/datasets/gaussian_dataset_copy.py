...
class Dataset(data.Dataset):
    def __init__(self, data_root, human, ann_file, split):
        ...
        # if cfg.volume_weight:
        #     self.canonical_bbox = self.skeleton_to_bbox(self.joints)
        #     self.motion_weights_priors = \
        #         approx_gaussian_bone_volumes(
        #             self.joints,   
        #             self.canonical_bbox['min_xyz'],
        #             self.canonical_bbox['max_xyz'],
        #             grid_size=cfg.mweight_volume_size).astype('float32')
    ...
    def __getitem__(self, index):
        ...
        #     })
        #     assert np.all(ret['cnl_bbox_scale_xyz'] >= 0)
        # # transformation
        #world to smpl 
