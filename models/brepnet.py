...
def build_matrix_Psi(Xf, Xe, Xc, Kf, Ke, Kc):
    ...
    # Pft.size() = [ num_coedges x num_faces_in_kernel x num_face_features ] #拿这个举例，Xf有2451个，代表面特征，Ke是变换核索引，每个元素对2451个面做索引，得到最终的11325，2，64
    Pft = Xf[Kf]  #比如第一个是f，半边对面的索引，那么第一组就是每个半边对应的面是什么
    ...
    Pf = torch.flatten(Pft, 1)   #将每个核，如f，mf进行展平
    Pe = torch.flatten(Pet, 1)
    Pc = torch.flatten(Pct, 1)

    # Now we can concatentate these tensors to form Psi 
    # Psi = torch.cat([Pf, Pe, Pc], dim=1)  #11325，517 一堆送进去，是每个半边对应的面特征，边特征核半边特征一起送进去，然后对面分类，labels有2451个，test的时候
    Psi = torch.cat([Pc, Pe, Pf], dim=1)
    return Psi
...
def find_max_feature_vectors_for_each_face(Zf, Cf, Csf, device): #zf代表了11325个半边的特征
    """
    ...
    Zft = Zfpad[Cf]   #找到所有半边对应的特征，其中11325代表填充，小面不是所有都有30个半边的
    ...
    for Csingle_face in Csf:  #csf代表大面所对的半边，通过对大面所在的半边的特征的索引
        ...
        (Hbig_face, Hbig_face_argmaxs)  = torch.max(Zsingle_face, dim=0)  #得到大面所对的半边的特征，找到一个最大特征的半边特征，这个半边所获取的特征最能代表这个面
    ...
    Hf = torch.cat(Hf_array, dim=0)#感觉就是排序，先小面再大面
...
class BRepNetLayer(LightningModule):
    ...
    def __init__(self, num_mlp_layers, input_size, output_size, dropout=None):  #out84
        """
    ...
    def forward(self, Xf, Xe, Xc, Kf, Ke, Kc, Ce, Cf, Csf):
        ...
        Psi = build_matrix_Psi(Xf, Xe, Xc, Kf, Ke, Kc)   #build之后先是517，然后就是1092，13个84了

        # Next the mlp is applied to Psi
        Z = self.mlp(Psi)  #517输入，252维了
        ...
        Hf = find_max_feature_vectors_for_each_face(Zf, Cf, Csf, self.device)   #这5个都是84
...
class BRepNetFaceOutputLayer(LightningModule):
    ...
    def forward(self, Xf, Xe, Xc, Kf, Ke, Kc, Ce, Cf, Csf):
        ...
        # Next the mlp is applied to Psi
        Z = self.mlp(Psi)  #output这里84，不是三倍了，11325，84

        # Finally use max pooling to combine the coedge
        # activations in Z to build the logits for faces
        Hf = find_max_feature_vectors_for_each_face(Z, Cf, Csf, self.device)  #缩到2451
        return Hf
...
class BRepNet(LightningModule):
    ...
    def __init__(self, opts):
        ...
        mlp_input_size = (num_faces_per_kernel*num_face_features) + \
                         ...
                         (num_coedges_per_kernel*num_coedge_features)   #517，就是psi的size

        # Subsequently the size depends only on the number of filters and
        # number of faces, edges and coedges in the kernel
        mlp_hidden_size = num_filters*(num_faces_per_kernel + num_edges_per_kernel + num_coedges_per_kernel)#84*（2+5+6）=1092

        # Create the layers of the network
        self.layers = nn.ModuleList()   #517，252--252，252--1092，252--252，252.....
        ...
        for l in range(2, opts.num_layers): #5
            self.layers.append(BRepNetLayer(num_mlp_layers, mlp_hidden_size, num_filters, dropout)) #2，
        ...
        self.classification_layer = nn.Linear(num_filters,num_classes)#8
    ...
    def create_face_embeddings(self, Xf, Gf, Xe, Ge, Xc, Gc, Kf, Ke, Kc, Ce, Cf, Csf):
        ...
        for i, layer in enumerate(self.layers):
            Xf, Xe, Xc, Kf, Ke, Kc, Ce, Cf, Csf = layer(Xf, Xe, Xc, Kf, Ke, Kc, Ce, Cf, Csf)  #每个面的最大特征的半边对应的特征重新赋值给这个面的特征
    ...
    def brepnet_step(self, batch, batch_idx, save_segmentation_output):
        ...
        face_embeddings = self.create_face_embeddings(Xf, Gf, Xe, Ge, Xc, Gc, Kf, Ke, Kc, Ce, Cf, Csf)#N,84

        # The tensor logits is now size [ num_faces_in_batch x num_classes ]
        segmentation_scores = self.classification_layer(face_embeddings)#N,8
        ...
        # print(max(predicted_classes),min(predicted_classes))
    ...
    def training_step(self, batch, batch_idx):
        ...
        print("loss: ",output["loss"],"acc: ",output["accuracy"])
