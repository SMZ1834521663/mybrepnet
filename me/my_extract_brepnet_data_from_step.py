...
class BRepNetExtractor:
    ...
    def process(self):
        ...
        #############
        dual_graph,edge_to_face=self.get_dual_graph(body)#获取对偶图
        #############
        face_features = self.extract_face_features_from_body(body, entity_mapper)#6，7
        edge_features = self.extract_edge_features_from_body(body, entity_mapper)#10，10
        coedge_features = self.extract_coedge_features_from_body(body, entity_mapper)#20，1

        face_point_grids = self.extract_face_point_grids(body, entity_mapper)#6，7，10，10
        assert face_point_grids.shape[1] == 7
        coedge_point_grids = self.extract_coedge_point_grids(body, entity_mapper)#20，12，10
        assert coedge_point_grids.shape[1] == 12

        coedge_lcs = self.extract_coedge_local_coordinate_systems(body, entity_mapper)#20，4，4
        coedge_reverse_flags = self.extract_coedge_reverse_flags(body, entity_mapper)#20，

        next, mate, face, edge  = self.build_incidence_arrays(body, entity_mapper)#20，  
    ...
    def get_dual_graph(self, body):
        top_exp = TopologyUtils.TopologyExplorer(body, ignore_orientation=True)

        face_index=dict([])
        for i,face in enumerate(top_exp.faces()):#哪个面对应哪个数字
            face_index[i]=face

        dual_graph=dict([])
        for i,face in enumerate(top_exp.faces()): #对偶图
            dual_graph[i]=[]

        edge_to_face=dict([])
        for i,edge in enumerate(top_exp.edges()):
            edge_to_face[i]=[]

        for ei,edge in enumerate(top_exp.edges()):
            faces_of_one_edge=top_exp.faces_from_edge(edge)
            faces=[face for face in faces_of_one_edge]
            if(len(faces)==2):   
                index1=-1
                index2=-1
                for fi,face in enumerate(top_exp.faces()):#判断面的索引是什么
                    if(face_index[fi].IsEqual(faces[0])):
                        index1=fi
                    if(face_index[fi].IsEqual(faces[1])):
                        index2=fi
                dual_graph[index1].append(index2)
                dual_graph[index2].append(index1)

                edge_to_face[ei].append(index1)
                edge_to_face[ei].append(index2)
            if(len(faces)==1):#侧向边，内边可以加上，就是自己和自己相连
                index=0
                for fi,face in enumerate(top_exp.faces()):
                    if(face_index[fi].IsEqual(faces[0])):
                        index=fi
                dual_graph[index].append(index)   
                
                edge_to_face[ei].append(index)
            if(len(faces)>2):  #一般不会出现这个情况
                print("error,face num is over 2")
                assert False

        return dual_graph , edge_to_face
    ...
    def load_body_from_step(self):
        ...
        shape = reader.OneShape()  #返回shappe对象
        return shape
    ...
    def extract_features_from_face(self, face):
        face_features = []
        for feature in self.feature_schema["face_features"]: #特征模式,all.json里的，利用拓扑学库的这几类给这些面打特征，也就是说直接分了一次特征
            if feature == "Plane":
    ...
    def area_feature(self, face):
        ...
        return geometry_properties.Mass() #质量
    ...
    def extract_face_point_grid(self, face):
        ...
        points = uvgrid(face, num_u, num_v, method="point")#3
        normals = uvgrid(face, num_u, num_v, method="normal")#3
        mask = uvgrid(face, num_u, num_v, method="inside")#1
    ...
    def extract_coedge_point_grids(self, body, entity_mapper):
        ...
        The values are
            ...
            - tx, ty, tz (tangent of the curve, oriented to match the coedge)切线
            - Lx, Ly, Lz (Normal for the left face)双面法线
    ...
    def extract_coedge_local_coordinate_systems(self, body, entity_mapper):
        ...
        This is a homogeneous transform matrix from local to global coordinates局部坐标到全局坐标
        """
        solid = create_occwl(body)
        top_exp = TopologyUtils.TopologyExplorer(body, ignore_orientation=False)
        coedge_lcs = []
